...
import {
  BimButton,
  BimIconButton,
} from '@bim-core/shadcn-ui/components/button';
import { BimScrollbar } from '@bim-core/shadcn-ui/components/scrollbar';
import {
  Sheet,
  SheetClose,
  SheetContent,
  ...
  SheetTitle,
  SheetTrigger,
} from '@bim-core/shadcn-ui/components/ui/sheet';
  ...
  <Sheet v-model:open="openModal">
    ...
    <SheetContent :style="contentStyle" class="!w-full pb-12 sm:rounded-l-lg">
      ...
      <div class="h-full pb-16">
        <BimScrollbar class="h-full" shadow>
          <slot></slot>
        </BimScrollbar>
      </div>
      <SheetFooter v-if="showFooter || slots.footer" as-child>
        ...
        >
          ...
          <template v-else>
            <SheetClose as-child>
              <BimButton class="mr-2" variant="outline">
                {{ cancelText }}
              </BimButton>
            </SheetClose>
            <BimButton @click="handlerSubmit">{{ submitText }}</BimButton>
          </template>
        </div>
      </SheetFooter>
    </SheetContent>
